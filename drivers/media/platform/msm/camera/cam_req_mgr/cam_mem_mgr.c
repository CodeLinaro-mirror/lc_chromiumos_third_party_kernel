	...
	size_t *len)
{
	...
	*vaddr = (uintptr_t)dma_buf_vmap(dmabuf);
	...
	/*
	 * Code could be simplified if dma_buf_vmap is
	 * available. This workaround takes the avandaage that alloc
	...
	int *fd)
{
	...
	*buf = cbm_alloc_buffer(len, flags);
	...
	enum cam_smmu_mapping_client client)
{
	...
	if (tbl.bufq[idx].dma_buf)
		cbm_free_buffer(tbl.bufq[idx].dma_buf);
	...
	struct cam_mem_mgr_memory_desc *out)
{
	...
	buf = cbm_alloc_buffer(inp->size, inp->flags);
	if (!buf) {
