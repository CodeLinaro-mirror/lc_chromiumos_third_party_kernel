...
static int dcc_probe(struct platform_device *pdev)
{
	...
	/* Set SID 0x0500 to S2CR bypass as mentioned in the mapping */
	ret = iommu_request_dm_for_dev(dev);
	if (ret) {
		dev_err(dev, "Request to direct map failed: %d\n", ret);
		goto err;
	}
...
static void dcc_shutdown(struct platform_device *pdev)
{
	int ret;
