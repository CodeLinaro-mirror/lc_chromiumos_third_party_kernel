...
static int sof_hdmi_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	list_for_each_entry(dai_link, &rtd->card->dai_link_list, list) {
							  ...
							  int hdmi_num)
{
	...
	if (sof_cs42l42_quirk & SOF_SPEAKER_AMP_PRESENT) {
		...
		if (sof_cs42l42_quirk & SOF_MAX98357A_SPEAKER_AMP_PRESENT) {
			links[id].codec_name = "MX98357A:00";
			links[id].codec_dai_name = "HiFi";
		...
		links[id].platform_name = "0000:00:0e.0",
	...
	links[id].id = id;
	links[id].codec_name = "i2c-10134242:00";
	links[id].codec_dai_name = "cs42l42";
	links[id].platform_name = "0000:00:0e.0",
	...
	for (i = 0; i < dmic_be_num; i++) {
		...
		links[id].codec_name = "snd-soc-dummy",
		links[id].codec_dai_name = "snd-soc-dummy-dai",
#else
...
static int sof_cs42l42_remove(struct platform_device *pdev)
{
	...
	list_for_each_entry(component, &card->component_dev_list, card_list) {
		if (!strcmp(component->name, "i2c-10134242:00")) {
